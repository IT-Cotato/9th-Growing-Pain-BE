...
@RequestMapping("/api/post/{post-id}/saves")
@Slf4j
public class PostSaveController {
    ...
    @ApiResponse(content = @Content(schema = @Schema(implementation = Response.class)))
    @DeleteMapping("/{post-save-id}")
    @ResponseStatus(HttpStatus.OK)
    public Response<?> deleteSavePost(@PathVariable("post-id") Long postId,
                                      @PathVariable("post-save-id") Long postSaveId,
                                      @AuthenticationPrincipal Long memberId) {
        log.info("게시글 {} 저장 취소한 memberId: {}", postId, memberId);
        postSaveService.deleteSavePost(postId, postSaveId, memberId);
